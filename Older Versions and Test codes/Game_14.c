...
void evaluateComputerMove(Player *player, int frequencies[], int rec_n);
void diceReroll_plyr(Player *player);
void frequency_up (Player *player, int frequencies[], int *rec_n);

int main() {
    srand(time(NULL));// Seed the random number generator
    int rec_n = 0; // Recursion times
...
/////////////////////////////////////////////////////////////////////////////////
        ...
        evaluateComputerMove(&computer, frequencies, rec_n); // Evaluate and choose the best move
...
void frequency_up (Player *player, int frequencies[], int *rec_n){
    //int recursion_depth = *n;// Use this for all scoring calculations
    if (*rec_n >= 2) {
    ...
    for (int i = 2; i<5; i++){
        if (frequencies[i] >= 3 && *rec_n < 2) {
            ...
            *rec_n += 1;
            frequency_up (player, frequencies, rec_n);
        }
...
void evaluateComputerMove(Player *player, int frequencies[], int rec_n) {
    ...
    for (int i = 0; i < 6; i++) {
        if (!player->usedCategories[i]){
            //if (frequencies[i] >= 0) { // Check for at least 3 dice with the same value
                score = frequencies[i] * (i + 1); // Dice value times its frequency
