...
void evaluateComputerMove(Player *player, int frequencies[]) {
    ...
    /*for (int i = 0; i < 6; i++) { // Categories 0–5 (Ones to Sixes)
        ...
        // Upper Section (Ones to Sixes)
    for (int i = 0; i < 6; i++) {
        if (frequencies[i] >= 3) { // Check for at least 3 dice with the same value
            int score = Upper_sec_combi(player, i + 1); // Compute score for category (dice value = i + 1)
