...
void evaluateComputerMove(Player *player, int frequencies[], int n);
...
/////////////////////////////////////////////////////////////////////////////////
        ...
        evaluateComputerMove(&computer, frequencies, 0); // Evaluate and choose the best move
    ...
    for (int i = 2; i<5; i++){
        if (frequencies[i] >= 3 ) {
            for (int k = 0; k<5; k++){
                if(player->dice[k] != (i+1)){
                    player->dice[k] = (rand() % 6) + 1;
                     ...
                     // Update frequencies after rerolling
                }
            }
            diceSort(player);
            printDice(player);
            ...
            //if (recursion_depth < 3){
                //evaluateComputerMove(player, frequencies, recursion_depth + 1);
            //}
            //return;
            if (recursion_depth < 3){
                evaluateComputerMove(player, frequencies, recursion_depth + 1);}
    ...
    for (int i = 0; i < 6; i++) {
        if (!player->usedCategories[i]){
            ...
            // Update the best score and category if this score is better
                if (score > bestScore) {
                    bestScore = score;
                    bestCategory = i /*+ 1*/; // Align category numbering with 1-based index
                }
            //}
        }
    }
