...
void diceReroll_plyr(Player *player) {
    for (int i = 0; i < 2; i++) {
        int Y, reroll[NUM_DICE] = {0};
        printf("Reroll? (Enter 1 for YES and 0 for NO): ");
        scanf("%d", &Y);
        if (Y == 1) {
            printf("Which dice do you want to roll? (Enter 1 for YES and 0 for NO): ");
...
int Ofa_kind(Player *player, int n) {
    int counts[NUM_SIDES] = {0}, sum = 0;
    for (int i = 0; i < NUM_DICE; i++) {
        counts[player->dice[i] - 1]++;
        sum += player->dice[i];
    }
...
void evaluateComputerMove(Player *player, int frequencies[]) {
    ...
    int score = 0;         // Temporary score for each category
    ...
    if (!player->usedCategories[10]) {
        if ((frequencies[0] == 1 && frequencies[1] == 1 && frequencies[2] == 1 &&
             frequencies[3] == 1 && frequencies[4] == 1) ||
            (frequencies[1] == 1 && frequencies[2] == 1 && frequencies[3] == 1 &&
             frequencies[4] == 1 && frequencies[5] == 1)) {
            score = 40; 
