...
void evaluateComputerMove(Player *player, int frequencies[], int recurs_n);  // recurs_n because in every round, copy of recurs_n will pass
...
void diceReroll_plyr (Player *player) {
    for (int i = 0; i < 2;i++){
            ...
            if (Y == 1) {
                ...
                } printDice(player); // When calling printDice, you should pass the pointer directly, not the address of the pointer.
            }
            else if (Y == 0) {
                //printDice(&human);
                break;
...
void evaluateComputerMove(Player *player, int frequencies[], int recurs_n) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (!player->usedCategories[i]){
            ...
            // Update the best score and category if this score is better
             /*   if (score > bestScore && frequencies[i] == 1 && (player->usedCategories[10] && player->usedCategories[11])) { 
                    // Scenes like when small & large straight chose before and arrays like (1, 2, 3, 4, 5), (2, 3, 4, 5, 6), (1, 3, 4, 5, 6),..... 
                    bestScore = score;
                    bestCategory = i ; // Align category numbering with 1-based index
                } */
            
/*  If a straight (small or large) is chosen, the AI might prioritize choosing those since they provide higher guaranteed scores (30 or 40 points).
But if both straights are already used, the AI may consider picking an Upper Section category with a single occurrence of a number to avoid getting stuck with no valid choices. */
